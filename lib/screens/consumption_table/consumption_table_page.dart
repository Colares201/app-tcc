...
class ConsumptionTablePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        // Escuta os últimos 15 registros
        stream: powerLogRef.orderByChild('timestamp').limitToLast(15).onValue,
        builder: (context, AsyncSnapshot<DatabaseEvent> snapshot) {
          if (snapshot.hasError) {
            return Center(child: Text('Erro ao carregar dados: ${snapshot.error}'));
          }
          ...
          // *** CORREÇÃO: Itera sobre os filhos (children) do snapshot ***
          final snapshotData = snapshot.data?.snapshot;

          if (snapshotData != null) {
            for (final child in snapshotData.children) {
              final value = child.value;

              if (value is Map && value.containsKey('timestamp') && value.containsKey('value')) {
                // A conversão de dynamic para int/num deve ser segura
                final int timestamp = value['timestamp'];
                final num powerValue = value['value'];
            ...
            // Não usamos logs.reversed.toList()
            // A ordem é garantida pela iteração dos children do snapshot (do mais antigo para o mais novo)
          }
